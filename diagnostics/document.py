...
('Tc', '', 0, 49, '', '', 9.23, 12.71, '', '', 'Tc: green', '', 232749), 
...
def sendDrRez(danP, zhP, davnZbP, plsP, anDsP, anOperP, alLsP, alOtSubP, vrPrP, OAKzP, BXzP, dopDanP, prof, z8ds = 778):
    import docx
    doc = docx.Document()
    from datetime import datetime  
    c = datetime.now()
    if danP[0] == 'пол: мужской':
        NameFile = 'Запрос_на_консультацию_'+str(c.year)+'.'+str(c.month)+'.'+str(c.day)+'.'+str(c.hour)+'.'+str(c.minute)+'.'+str(c.second)+'.1.'+str(danP[1])+'.1'
    if danP[0] == 'пол: женский':
        NameFile = 'Запрос_на_консультацию_'+str(c.year)+'.'+str(c.month)+'.'+str(c.day)+'.'+str(c.hour)+'.'+str(c.minute)+'.'+str(c.second)+'.2.'+str(danP[1])+'.1'
    print(NameFile)
    p = doc.add_paragraph()
    runner = p.add_run(NameFile)
    runner.bold = True
    ymdhms = str(c.year)+'.'+str(c.month)+'.'+str(c.day)+'_'+str(c.hour)+':'+str(c.minute)
    doc.add_paragraph(ymdhms)
    print(ymdhms)
    o_obschSvedP = set()
    if danP[0] == 'пол: мужской':
    ...
    doc.add_paragraph('Дата, месяц, год рождения:')
    if dopDanP[0].replace(' ', '') != '':
        doc.add_paragraph('WhatsApp: '+str(dopDanP[0]))
    else:
        doc.add_paragraph('WhatsApp: данные не предоставлены.')
    if dopDanP[1].replace(' ', '') != '':
        doc.add_paragraph('Telegram: '+str(dopDanP[1]))
    else:
        doc.add_paragraph('Telegram: данные не предоставлены.') 
    if dopDanP[2].replace(' ', '') != '':
        doc.add_paragraph('Телефон: '+str(dopDanP[2]))
    else:
        doc.add_paragraph('Телефон: данные не предоставлены.') 
    if dopDanP[3].replace(' ', '') != '':
        doc.add_paragraph('Email: '+str(dopDanP[3]))
    else:
        doc.add_paragraph('Email: данные не предоставлены.') 
    if prof[0].replace(' ', '') != '':
        doc.add_paragraph('Профессия: '+str(prof[0]))
    else:
        doc.add_paragraph('Профессия: данные не предоставлены.') 
    if danP[0] == 'пол: мужской':
        doc.add_paragraph('Пол: мужской')
    if danP[0] == 'пол: женский':
        doc.add_paragraph('Пол: женский')
    doc.add_paragraph('Возраст, лет: '+str(danP[1]))
    doc.add_paragraph('Вес, кг: '+str(danP[2]))
    doc.add_paragraph('Рост, см: '+str(danP[3]))
    if danP[4] != '':
        doc.add_paragraph('Беременность, недели: '+str(danP[4]))
    else:
        doc.add_paragraph('Беременности нет в настоящее время.')
    ...
    if o_sm8zhP != set():
        print('\nЖалобы:')
        par1 = doc.add_paragraph('Жалобы: ')
        for q in f_sm8zhP:
            print(f"  {q}")
            if f_sm8zhP.index(q) != len(f_sm8zhP)-1:
                par1.add_run('\n - '+q+';')
            else:
                par1.add_run('\n - '+q+'.')
    else:
        print('\nЖалобы не предъявлены.')
        doc.add_paragraph('Жалобы не предъявлены.')
    if davnZbP[0].replace(' ', '') != '':
        doc.add_paragraph('Давность заболевания: '+str(davnZbP[0]))
    else:
        doc.add_paragraph('Давность заболевания: данные не предоставлены.')
    if plsP[0].replace(' ', '') != '':
        doc.add_paragraph('Принимаемые лекарства: '+str(plsP[0]))
    else:
        doc.add_paragraph('Принимаемые лекарства: данные не предоставлены.')
    if anDsP[0].replace(' ', '') != '':
        doc.add_paragraph('Ранее перенесённые заболевания: '+str(anDsP[0]))
    else:
        doc.add_paragraph('Ранее перенесённые заболевания: данные не предоставлены.')
    if anOperP[0].replace(' ', '') != '':
        doc.add_paragraph('Ранее перенесённые операции: '+str(anOperP[0]))
    else:
        doc.add_paragraph('Ранее перенесённые операции: данные не предоставлены.')
    if alLsP[0].replace(' ', '') != '':
        doc.add_paragraph('Аллергия на лекарства: '+str(alLsP[0]))
    else:
        doc.add_paragraph('Аллергия на лекарства: данные не предоставлены.')
    if alOtSubP[0].replace(' ', '') != '':
        doc.add_paragraph('Аллергия на продукты питания и другие вещества: '+str(alOtSubP[0]))
    else:
        doc.add_paragraph('Аллергия на продукты питания и другие вещества: данные не предоставлены.')
    if vrPrP[0].replace(' ', '') != '':
        doc.add_paragraph('Вредные привычки: '+str(vrPrP[0]))
    else:
        doc.add_paragraph('Вредные привычки: данные не предоставлены.')
    from datetime import datetime
                        ...
                        if e[2] == '' and e[3] == '' or e[2] == '' and e[3] != '' and danP[1] != '' and danP[1] < e[3] or e[2] != '' and e[3] == '' and danP[1] != '' and danP[1] >= e[2] or e[2] != '' and e[3] != '' and danP[1] != '' and danP[1] >= e[2] and danP[1] < e[3]:
                            if e[4] == '' and e[5] == '' or e[4] == '' and e[5] != '' and danP[4] != '' and danP[4] < e[5] or e[4] != '' and e[5] == '' and danP[4] != '' and danP[4] >= e[4] or e[4] != '' and e[5] != '' and danP[4] != '' and danP[4] >= e[4] and danP[4] < e[5]:
                                if e[6] != '' and q[1] < e[6]:
                                    o_sm8v8zP.add(e[9])
                                    if 'снижение' in e[9]:
                                        oKo_v_z_edIzm_interP.add((q[0], str(q[1]), q[2], '(снижение)'))
                                if e[6] != '' and q[1] >= e[6] and e[7] == '' or e[6] != '' and q[1] >= e[6] and e[7] != '' and q[1] < e[7]:
                                    o_sm8v8zP.add(e[10]) 
                                    if 'норма' in e[10]:
                                        oKo_v_z_edIzm_interP.add((q[0], str(q[1]), q[2], '(норма)'))
                                if e[7] != '' and q[1] >= e[7]:
                                    o_sm8v8zP.add(e[11])   
                                    if 'повышение' in e[11]:
                                        oKo_v_z_edIzm_interP.add((q[0], str(q[1]), q[2], '(повышение)'))
    o_sm8v8zP.discard('')  
    ...
    for q in f_BX:
        for w in oKo_v_z_edIzm_interP:
            if q == w[0]:
                fKo_BXp.append(w)
    if fKo_OAKp != []:
        print('Общий анализ крови:')
        parOAK = doc.add_paragraph('Общий анализ крови: ')
        for q in fKo_OAKp:
            print(f"  {q[0]}: {q[1]} {q[2]}, {q[3]}")
            if fKo_OAKp.index(q) != len(fKo_OAKp)-1:
                parOAK.add_run('\n - '+q[0]+': '+q[1]+' '+q[2]+' '+q[3]+';')
            else:
                parOAK.add_run('\n - '+q[0]+': '+q[1]+' '+q[2]+' '+q[3]+'.')
    else:
        print('Общий анализ крови не предоставлен.\n')
        doc.add_paragraph('Общий анализ крови не предоставлен.')
    if fKo_BXp != []:
        parBX = doc.add_paragraph('Биохимический анализ крови: ')
        for q in fKo_BXp:
            if fKo_BXp.index(q) != len(fKo_BXp)-1:
                parBX.add_run('\n - '+q[0]+': '+q[1]+' '+q[2]+' '+q[3]+';')
            else:
                parBX.add_run('\n - '+q[0]+': '+q[1]+' '+q[2]+' '+q[3]+'.')
    else:
        doc.add_paragraph('Биохимический анализ крови не предоставлен.')
    o_sm8uP = set()
    o_sm1 = set()
    ...
    for q in o_sm1:
        for w in sm8uR:
            if q == w[0] or q == w[1]:
                o_sm8uP.add(q)
    if o_sm8uP != set():           
        parSmU = doc.add_paragraph('ВНИМАНИЕ! Имеются симптомы, которые могут быть проявлением срочного, неотложного состояния, угрожающего здоровью и жизни!\nСледует СРОЧНО обратиться за медицинской помощью и сообщить об этих симптомах:')
        for q in sorted(o_sm8uP):
            print(f"  {q}")
            if sorted(o_sm8uP).index(q) != len(sorted(o_sm8uP))-1:
                parSmU.add_run('\n - '+q+';')
            else:
                parSmU.add_run('\n - '+q+'.')
    doc.add_paragraph('Индекс массы тела: '+str(round(BMI, 2))+' - '+BMI_interp+'.')
    o_sm8kP = set()
    o8ko_sm8k_smP = set()
    for q in o_sm8zhP:
        for w in zhR:
    ...
    f8ko8sort8vosh8z8ds_ds8mP_z = []
    for q in range(z8ds):
        if z8ds - q <= len(f8ko8sort8vosh_ds8mP_z):
            f8ko8sort8vosh8z8ds_ds8mP_z.append(f8ko8sort8vosh_ds8mP_z[q - z8ds])
    f8ko8sort8nish8z8ds_ds8mP_z = list(reversed(f8ko8sort8vosh8z8ds_ds8mP_z))  
    g_dsP_ko8smP = {}
    for q in f8ko8sort8nish8z8ds_ds8mP_z:
        o8smP = set()
        for w in sm_ds:
    ...
    f8sort8nish8z8ds_ds8mP = []
    for q in f8ko8sort8nish8z8ds_ds8mP_z:
        f8sort8nish8z8ds_ds8mP.append(q[0])
    if f8sort8nish8z8ds_ds8mP != []:
        parDsSm = doc.add_paragraph('На основании предъявленных жалоб и результатов обследования обнаружены признаки следующих диагнозов и состояний.\n!!! ВНИМАНИЕ: ДАННЫЕ ДИАГНОЗЫ И СОСТОЯНИЯ ПРЕДВАРИТЕЛЬНЫЕ И ДОЛЖНЫ БЫТЬ ПЕРЕПРОВЕРЕНЫ !!!\nНа первых местах более вероятные диагнозы и состояния:')
        for q in f8sort8nish8z8ds_ds8mP:
            for w, e in g_dsP_ko8smP.items():
                if q == w:
                    print(f"  {q} {str(e)}")
                    if f8sort8nish8z8ds_ds8mP.index(q) != len(f8sort8nish8z8ds_ds8mP)-1:
                        parDsSm.add_run('\n - '+q+' '+str(e).replace("'", "")+';')
                    else:
                        parDsSm.add_run('\n - '+q+' '+str(e).replace("'", "")+'.')
    else:
        doc.add_paragraph('На основании предъявленных жалоб и результатов обследования подозрений на какие-либо диагнозы и патологические состояния не выявлено.')
    c = datetime.now()
    doc.save(NameFile+'.docx')

    return NameFile

# 2024 0103 2147

def sendDrSm(danP, zhP, davnZbP, plsP, anDsP, anOperP, alLsP, alOtSubP, vrPrP, OAKzP, BXzP, dopDanP, prof):
    import docx
    doc = docx.Document()
    from datetime import datetime  
    c = datetime.now()
    if danP[0] == 'пол: мужской':
        NameFile = 'Запрос_на_консультацию_'+str(c.year)+'.'+str(c.month)+'.'+str(c.day)+'.'+str(c.hour)+'.'+str(c.minute)+'.'+str(c.second)+'.1.'+str(danP[1])+'.2'
    if danP[0] == 'пол: женский':
        NameFile = 'Запрос_на_консультацию_'+str(c.year)+'.'+str(c.month)+'.'+str(c.day)+'.'+str(c.hour)+'.'+str(c.minute)+'.'+str(c.second)+'.2.'+str(danP[1])+'.2'
    print(NameFile)
    p = doc.add_paragraph()
    runner = p.add_run(NameFile)
    runner.bold = True
